...
const visitor = {
  DebuggerStatement (path, state) {
    if (state) {
      ...
      if (typeof state.opts.debugger === 'undefined') {
        state.opts.debugger = defaultOptions.debugger || true
      }
  ...
  ExpressionStatement (path, state) {
    if (state) {
      ...
      if (
        chainGet(path).node.expression.type() === 'CallExpression' &&
        chainGet(path).node.expression.callee()
      ) {
        // remove alert
        ...
        if (
          ...
          chainGet(path).node.expression.callee.object.name() === 'console'
        ) {
          if (state && state.opts) {
            if (typeof state.opts.console === 'undefined') {
              state.opts.console = defaultOptions.console || true
            }
          }
  ...
  FunctionDeclaration (path, state) {
    if (
      !state ||
      typeof state.opts === 'undefined' ||
      !state.opts.debugFn ||
      typeof state.opts.debugFn !== 'string'
    )
      return
    if (
      chainGet(path).node.id.type() === 'Identifier' &&
      path.node.id.name === state.opts.debugFn
    ) {
      path.remove()
    }
  },
